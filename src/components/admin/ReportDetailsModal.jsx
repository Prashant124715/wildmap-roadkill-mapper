        ...
        >
            ...
            <div className="h-64 md:h-1/2 relative bg-black flex items-center justify-center">
              {report.imageUrl ? (
                ...
                <div className="text-center text-gray-500">
                  ...
                  <p className="text-xs uppercase tracking-widest">No Image Provided</p>
                </div>
              )}
            </div>
            ...
            <div className="flex-1 p-6 bg-gradient-to-b from-black to-[#0a0a0a]">
              <div className="flex justify-between items-center mb-4">
                ...
                {report.aiScore === undefined && report.imageUrl && (
                  ...
                  >
                    {isAnalyzing ? <><Loader2 size={12} className="animate-spin"/> Analyzing...</> : 'Run Analysis'}
                  </button>
                )}
              </div>
              ...
              {report.aiScore !== undefined ? (
                ...
                <div className="h-full flex items-center justify-center">
                  <p className="text-xs text-gray-600 text-center">
                    {report.imageUrl ? "Click 'Run Analysis' to evaluate the image using the simulated AI model." : "AI analysis requires an image."}
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="w-full md:w-1/2 p-6 md:p-8 flex flex-col overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-4 mb-8">
              <div>
                <h3 className="text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-1">Reported Species</h3>
                <p className="text-lg font-bold text-white">{report.species}</p>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <h3 className="text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-1 flex items-center gap-1"><MapPin size={12}/> Location</h3>
                  <p className="text-sm text-gray-300 font-mono">{report.location}</p>
                </div>
                <div>
                  <h3 className="text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-1 flex items-center gap-1"><Calendar size={12}/> Date & Time</h3>
                  <p className="text-sm text-gray-300">{new Date(report.timestamp).toLocaleString()}</p>
                </div>
              </div>

              <div>
                <h3 className="text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-1">User Details</h3>
                <p className="text-sm text-gray-300">Submitted by: <span className="font-medium text-white">{report.userId}</span></p>
              </div>
            ...
            <div className="mt-auto pt-6 border-t border-white/10">
              <h3 className="text-[10px] text-gray-500 uppercase tracking-widest font-bold mb-3">Admin Decision</h3>
